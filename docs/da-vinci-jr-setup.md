...
There are two board-specific conflicts that must be handled physically.
...
The board reverse-engineering maps PA11 to the onboard MX25L3206E flash chip-select and PA12 through PA14 to the same flash bus. Do not bridge PB2 to PA11 while that flash can still be selected. Isolate the flash CS from PA11 and hold the flash CS inactive/high. The flash itself does not need to be removed. Once its CS is permanently inactive, sharing PA12 through PA14 is safe and RepRapFirmware does not use that flash.

Second, PA26 is normally SD DAT2. This port switches the SD socket to one-bit HSMCI so PA26 can be used for WiFi. SD remains available through CMD, CLK, and DAT0 on PA28, PA29, and PA30.

The traced board also assigns PC24 and PB14 to the stock right/left laser nets. This configuration dedicates those pins to ESP reset and enable, so those laser functions cannot be used at the same time.
...
Module 2 of RepRapFirmware's `M997` updater is assigned to the LPC1115. Put `Lpc1115Firmware.bin` in `0:/firmware/` and run:

```gcode
M997 S2
```

A different filename can be selected with, for example:

```gcode
M997 S2 P"test-lpc.bin"
```

Before flashing, the normal `M997` path switches off all heaters and disables the stepper drives. The updater checks the image and takes the normal SAM-to-LPC protocol offline. It then holds LPC PIO0_1 low while resetting PIO0_0, synchronizes with the LPC1115 ROM UART ISP, unlocks flash, checks the LPC1115 part ID, erases the flash, programs it in 512-byte blocks, and compares every programmed block.

Sector zero is deliberately written last, with the vector-table block at address zero written last of all. If power or UART communication is lost during an update, the LPC therefore remains recoverable through ROM ISP instead of booting a partially programmed application. Re-running `M997 S2` re-enters ROM ISP and retries the update.
...
After a successful update the SAM releases the ISP strap, resets the LPC into the application, and the normal LPC protocol reconnects and replays the heater/GPIO configuration.

### Updating the LPC over WiFi

WiFi is the user-facing transport. UART ISP remains the reliable physical programming transport between the two MCUs. Once Duet WiFi is connected, upload `Lpc1115Firmware.bin` to the board's `firmware` directory through the web interface and issue `M997 S2` from the web console. No SWD connection is needed for later LPC firmware updates.

SWD remains the lowest-level recovery path if the SAM firmware itself is unavailable or the reset/ISP/UART hardware is damaged.
