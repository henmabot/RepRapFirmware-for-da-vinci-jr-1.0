...
bool SendUuencodedBlock(const uint8_t *data, size_t length) noexcept
{
	...
	return WaitForLine("OK", CommandTimeout);
}
...
bool ProgramBlock(FileStore& firmware, FilePosition imageLength, uint32_t flashOffset) noexcept
{
	...
	// The boot ROM remaps the first 64 bytes while it is running, so compare the
	// remainder of block zero exactly as lpc21isp does.
	if (flashOffset == 0)
	{
		return SendFormattedCommand("M %lu %lu %lu\r\n", 64u, RamBuffer + 64u, TransferSize - 64u);
	}
	return SendFormattedCommand("M %lu %lu %lu\r\n", flashOffset, RamBuffer, TransferSize);
}
...
bool ValidateFirmware(FileStore& firmware, const char *filename, const StringRef& reply) noexcept
{
	...
	if (vectors[0] != RamTop || (resetVector & 1u) == 0 || (resetVector & ~1u) >= FlashSize || vectorSum != 0)
	{
...
bool Update(const StringRef& filenameRef) noexcept
{
	...
	{
		reprap.GetPlatform().Message(ErrorMessage, "LPC1115 firmware update failed; retry M997 S2 to re-enter ROM ISP\n");
	}
	return ok;
}
